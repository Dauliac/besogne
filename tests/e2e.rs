...
fn e2e_npm_install_skip_on_second_run() {
    ...
    assert!(stderr(&r2).contains("preconditions cached"), "2nd run should skip: {}", stderr(&r2));
}
...
fn e2e_cache_skip() {
    ...
    assert!(
        !err1.contains("preconditions cached"),
    ...
    let r2 = run_in(dir.path());
    assert!(r2.status.success());
    assert!(stderr(&r2).contains("preconditions cached"), "run 2 should skip: {}", stderr(&r2));
    assert_eq!(
...
fn e2e_cache_invalidate() {
    ...
    let r2 = run_in(dir.path());
    assert!(r2.status.success());
    assert!(!stderr(&r2).contains("preconditions cached"), "should NOT skip: {}", stderr(&r2));
    assert_eq!(
        std::fs::read_to_string(dir.path().join("marker.txt")).unwrap().lines().count(),
