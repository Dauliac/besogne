...
## Manifest format: inputs are a NAMED MAP

Inputs are `HashMap<String, Input>` — the key IS the name. NOT an array.
...
- `PluginInput` has no `key` — key IS the plugin reference
- `BinaryInput.name` optional — defaults to key
- `EnvInput.name` optional — defaults to key
...
- Input IDs: `<type>:<identifier>:<blake3_short>` (e.g., `binary:go:a1b2c3d4`)
...
- Extracts command names → validates against binary inputs
- Extracts `$VAR` refs → validates against env/extracted inputs
...
- `params | { ... }` contract + `produces = fun params => [native inputs]`
...
| `src/ir/types.rs` | IR types: `BesogneIR`, `ResolvedInput`, `ContentId` |
