...
fn parse_by_extension(path: &str, content: &str) -> Result<serde_json::Value, String> {
    ...
    match ext.as_str() {
        ...
        "toml" => {
            ...
            let json_str = serde_json::to_string(&toml_val)
                .map_err(|e| format!("cannot convert TOML to JSON: {e}"))?;
            serde_json::from_str(&json_str)
                .map_err(|e| format!("internal error converting TOML: {e}"))
        }
        _ => Err(format!("unsupported config file extension '.{ext}' (use .json, .yaml, .yml, or .toml)"))
    }
}
